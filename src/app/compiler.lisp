...
(defun compile-funcall (expr asm env nb-var)
  ...
  (let ((param (compile-funcall-parameter (cdr expr) asm env nb-var))
        ...
        (name-fun (car expr)))
    (append param  ;; ici c'est la compilation de mes arguments
            (list '(PUSH :FP) '(MOVE :SP :FP))  ;; Sauvegarde du Framepointeur
            (list (list 'MOVE nbparam :R0) '(PUSH :R0))  ;; Sauvegarde du nombre d'arguments
            (list (list 'JSR name-fun)))))  ;; Ici je JUMP à la fonction avec retour
